...
// .NET implementation of: https://github.com/forax/design-pattern-reloaded/blob/master/src/main/java/memoizer/memoizer4.java
...
namespace DesignPatternReloaded.Memoizer
{
    ...
    public static class Memoizer4
    {

        private static Func<V, R> Memoize<V, R>(Func<V, Func<V, R>, R> func)
        {
            IDictionary<V, R> dict = new Dictionary<V, R>();
            Func<V, R>[] memoizer = new Func<V, R>[1];
            return memoizer[0] = value => dict.ComputeIfAbsent(value, v => func(v, memoizer[0]));
        }
